...
\title{Modeling Attention in Panoramic Video: \\A Deep Reinforcement Learning Approach}
%
%
...
\author{Mai~Xu*,~\IEEEmembership{Senior Member,~IEEE,}
        ...
        and~Zulin Wang% <-this % stops a space
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem  M. Xu, Y. Song, J. Wang, M. Qiao, L. Huo and Z. Wang are with the School of Electronic and Information
...
\begin{figure*}
	\begin{center}
      ...
      \subfigure[]{\includegraphics[width=0.58\columnwidth]{figures/introduction/Head-eps-converted-to}}
		\subfigure[]{\includegraphics[width=1.36\columnwidth]{figures/introduction/fig_one-eps-converted-to}}
        \vspace{-.5em}
...
To obtain the HM maps of panoramic video, the HM positions are predicted by estimating the HM scanpaths of several \textit{agents}. Similarly, in the saliency detection area, there exist some works \cite{Wang2011, Sun12, Liu_2013_ICCV, Jiang2016, Shao2017} of this response that predict eye movement scanpaths for static images.
In \cite{Wang2011} of this response, a computational model was developed to simulate the scanpaths of eye movements on natural images. The proposed model embeds three factors to guide eye movements sequentially, including reference sensory responses, fovea periphery resolution discrepancy, and visual working memory. Sun \textit{et al.} \cite{Sun12} proposed modelling both saccadic scanpaths and visual saliency of images, on the basis of super Gaussian component (SGC) analysis. Recently, data-driven approaches have been proposed to learn the scan-paths of eye movements on static images, such as the hidden Markov model in \cite{Liu_2013_ICCV} in this response and least-squares policy iteration (LSPI) in \cite{Jiang2016} in this response.
Most recently, deep learning has been utilized in \cite{Jiang2016, Shao2017} of this response for predicting the eye movement scanpaths in static images.
However, to our best knowledge, there is no work on predicting the HM scan-paths on panoramic video.


In this paper, a DRL approach is developed for predicting the actions of the HM scanpaths from multiple \textit{agents}.
The actions are decided based on the environment of the panoramicvideo content, the features of which are automatically learned and then extracted by a DNN. Thus, our approach takes advantage
...
With the support of the software development kit of HTC Vive, we recorded the posture data of each subject as they viewed the panoramic video sequences.
...
\begin{center}
  ...
  \tabincell{c}{Cate-\\gory} & Name & CC & \tabincell{c}{Cate-\\gory} & Name & CC & \tabincell{c}{Cate-\\gory} & Name & CC & \tabincell{c}{Cate-\\gory} & Name & CC \\
  \hline
  \multirow{10}{*}{\rotatebox{90}{Computer animation}} & AcerEngine & 0.891$\pm$0.058
&
...
%\vspace{-1em}
%	\begin{center}
%		\centerline{\includegraphics[width=1.5\columnwidth]{figures/experiment/scanpath_objective_result}}%of
%\vspace{-.5em}
...
Table \ref{CC-table} and \ref{NSS-table} tabulate the results of CC and NSS in predicting the HM maps of 15 test sequences, for our and other approaches.
...
\vspace{-1em}
	\begin{center}
		\centerline{\subfigure[Dancing]{\includegraphics[width=1.5\columnwidth]{figures/experiment_on_line/scanpath_Dancing}}}%of
\vspace{-1em}
        \centerline{\subfigure[KingKong]{\includegraphics[width=1.5\columnwidth]{figures/experiment_on_line/scanpath_Kingkong}}}%of
\vspace{-1em}
...
\begin{IEEEbiography}[{\includegraphics[width=1\linewidth]{figures/biography/MaiXu.pdf}}]{Mai Xu}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/biography/ZulinWang.pdf}}]{Zulin~Wang}
