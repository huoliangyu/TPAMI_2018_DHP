...
\begin{algorithm}
   \caption{Online-DHP-train}
   \label{online-DHP-algorithm-train}
\begin{algorithmic}
   \STATE {\bfseries Input:} Panoramic video and viewer's ground-truth HM position $(x^{n}_{1 \sim T},y^{n}_{1 \sim T})$, where $T$ is the number of frame in the video;
...
\begin{algorithm}
   \caption{Online-DHP-predict}
   \label{online-DHP-algorithm-predict}
\begin{algorithmic}
