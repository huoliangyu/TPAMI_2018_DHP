...
\begin{figure*}
	\begin{center}
		\centerline{\includegraphics[width=2\columnwidth]{figures/database/direction-consistence1}}%of
	\end{center}
\end{figure*}
\begin{figure*}
	\begin{center}
		\centerline{\includegraphics[width=2\columnwidth]{figures/database/direction-consistence2}}%of
...
\begin{figure*}
	\begin{center}
		\centerline{\includegraphics[width=2\columnwidth]{figures/database/direction-consistence-distribution1}}%of
	\end{center}
\end{figure*}
\begin{figure*}
	\begin{center}
		\centerline{\includegraphics[width=2\columnwidth]{figures/database/direction-consistence-distribution2}}%of
...
$(x^n_t, y^n_t)$ & The ground-truth HM position of $n$-th subject at frame $t$ \\
$\mathbf{\pi}_t$ & The predicted probability distribution of HM direction at frame $t$, as the $policy$ of DRL $agent$ \\
$\alpha_t$ & The predicted HM direction at frame $t$, as the $action$ of DRL $agent$ \\
$\alpha^n_t$ & The ground-truth HM direction of the $n$-th subject at frame $t$ \\
$\nu_t$ & The predicted HM magnitude at frame $t$, as the $action$ of DRL $agent$ \\
$\nu^n_t$ & The ground-truth HM magnitude of $n$-th subject at frame $t$ \\
$r^{\alpha}_t$ & Estimated reward regarding to the predicted HM direction $\alpha_t$, as the $reward$ of DRL $agent$ \\
$r^{\nu}_t$ & Estimated reward regarding to the predicted HM direction $\nu_t$, as the $reward$ of DRL $agent$ \\
$p^n(\ast)$ & The probability of event $\ast$ happening for the $n$-th subjects\footnote{In our paper, the event is specified to: being at a HM position $(x_t,y_y)$, or taking a HM direction of $\alpha_t$}. \\
...
(1) At frame $t$, the DRL model obtains current $\textit{observation}$ from the FoV extractor, denoted as $o_t$.
In our work, $\textit{observation}$ $o_{t}$ is the content of FoV ($103^{\circ} \times 60^{\circ}$) projected to 2D region and then down-sampled to $42\times42$.\\
(2) Current $\textit{observation}$ $o_t$ and the feature of LSTM from the last frame $f_{t-1}$ are delivered to the DRL network in the HM scan-path predictor.\\
(3) The DRL network produces LSTM feature $f_{t}$ \cite{hausknecht2015deep}, HM scan-path magnitude $\hat{\nu}_{t}$ and policy $\hat{\pi}_{t}$. Here, $\hat{\pi}_{t}$ is the probability distribution over \textit{actions} of HM scan-path directions.
...
(4) Given $\hat{\pi}_{t}$, the HM scan-path predictor randomly selects an \textit{action}  $\hat{\alpha}_t$, with standard deviation $\varepsilon$ to ensure exploration. Here, \textit{action} $\hat{\alpha}_t$ models directions of the HM scan-path, including 8 discrete directions \textcolor{blue}{based on the ENU coordinate system}\cite{ENU}, i.e., $\{ 0^{\circ}, 45^{\circ}, \cdots, 315^{\circ} \}$.\\
(5) \textit{Environment} is updated with $\hat{\nu}_t$ and $\hat{\alpha}_t$. Specifically, the FoV extractor returns a new \textit{observation} $o_{t+1}$ according to the current HM position, and the reward estimator returns the corresponding \textit{reward} $r^{\nu}_t$ and $r^{\alpha}_t$ for estimating $\hat{\nu}_t$ and $\hat{\alpha}_t$, upon ground-truth HM scan-paths.\\
(6) A set of experiences $\{ o_{t}, \! f_{t-1},\! \hat{\nu}_t,\! \hat{\alpha}_t,\! r^{\nu}_{t},\! r^{\alpha}_{t} \}$ is stored in an experience buffer for frame $t$.\\
(7) \textcolor{red}{Once $t$ meets the terminal condition $T$, which is the number of frames for the video, all experiences in the buffer are delivered to the optimizer for updating the DRL network}.
