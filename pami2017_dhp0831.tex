...
$\{\mathbf{F}_t\}_{t=1}^{T}$ & The panoramic frames from $1$ to $T$, as the input to offline-DHP/online-DHP\\
$\{\mathbf{H}_t\}_{t=1}^{T}$ & The HM maps of frames from $1$ to $T$, as the output of offline-DHP\\
$\{m\}_{m=1}^{m}$ & The subjects from $1$ to $M$, with $m$ being the $m$-th subject/workflow\\
$\{n\}_{n=1}^{N}$ & The DRL workflows from $1$ to $N$, with $n$ being the $n$-th workflow\\
$\{\mathbf{o}^n_t\}_{t=1}^{T}$ & The FoV from $1$ to $T$, as the $observation$ of the $n$-th DRL workflow \\
$(x^m_t, y^m_t)$ & The ground-truth HM position of the $m$-th subject at frame $t$ \\
$(\hat{x}^n_t, \hat{y}^n_t)$ & The HM position predicted by the $n$-th workflow at frame $t$, with one workflow modelling a subject  \\
...
$\hat{\alpha}^n_t$ & The predicted HM direction at frame $t$ from the $n$-th DRL workflow, as the $action$ of the DRL $agent$ \\
$\alpha^m_t$ & The ground-truth HM direction of the $m$-th subject at frame $t$ \\
$\hat{\nu}^n_t$ & The predicted HM magnitude at frame $t$ from the $n$-th DRL workflow, as the $action$ of the DRL $agent$ \\
$\nu^m_t$ & The ground-truth HM magnitude of the $m$-th subject at frame $t$ \\
$r^{\alpha}_{n,t}$ & The estimated reward for deciding $\hat{\alpha}^n_t$, as the $reward$ of the $n$-th DRL workflow  \\
$r^{\nu}_{n,t}$ & The estimated reward for deciding $\hat{\nu}^n_t$, as the $reward$ of the $n$-th DRL workflow \\
$\mathbf{f}^n_{t}$ & The extracted LSTM feature at frame $t$,  as part of \textit{observed state} of the $n$-th DRL workflow \\
\end{tabular}
\end{table*}
...
Since \textit{Finding 1} has shown that the HM positions are highly consistent across different subjects,  we propose to generate HM maps for modeling human attention on panoramic video, seen as the output of our online-DHP approach. The HM map $\mathbf{H}_t$ of frame $t$ represents the probability of each pixel being the HM position.
Similar to saliency maps of 2D video, $\mathbf{H}_t$ is obtained by convoluting the predicted HM positions $\{(\hat{x}^n_t, \hat{y}^n_t)\}_{n=1}^{N}$ with a 2D Gaussian filter.
Because \textit{Finding 4} has found that the HM scan-paths of different subjects are consistent in more than one directions, the HM positions $\{({x}^m_t, {y}^m_t)\}_{m=1}^{M}$ of $M$ subjects may be different from each other. Accordingly, this paper assumes that the number of the predicted HM positions $N$ at each frame is equivalent to $M$, for predicting HM positions of all subjects.
Next, to obtain $(\hat{x}^n_t, \hat{y}^n_t)$, our offline-DHP approach estimates the HM positions of one subject through HM scan-path prediction of one DRL workflow.
Then, $N$ DRL workflows are run to obtain $N$ HM positions $\{(\hat{x}^n_t, \hat{y}^n_t)\}_{n=1}^{N}$ at frame $t$, corresponding to the ground-truth HM positions  $\{({x}^m_t, {y}^m_t)\}_{m=1}^{M}$ of all subjects at this frame.
...
In fact, $(\hat{x}^n_t, \hat{y}^n_t)$ of  one DRL workflow can be modeled by determining a series of \textit{actions}: $\hat{\alpha}^n_1,\hat{\alpha}^n_2,...,\hat{\alpha}^n_t$ and $\hat{\nu}^n_1,\hat{\nu}^n_2,...,\hat{\nu}^n_t$.
It is worth pointing out that it is reasonable to model $\{\hat{\alpha}^n_t\}_{t=1}^{T}$ and $\{\hat{\nu}^n_t\}_{t=1}^{T}$ as the \textit{actions} in the DRL workflow, since \textit{Findings 2} and \textit{3} have indicated that subjects are consistent in the magnitudes and directions of HM scan-paths.
As can be seen in Figure \ref{main-framework}, in a single DRL workflow, one HM scan-path is generated through the interaction between FoV extractor and HM scan-path predictor.
...
Figure \ref{train-framework} shows the proposed training model of DRL.
...
In addition, the \textit{reward} is estimated to evaluate the \textit{action} of the DRL model, which measures the similarity between the prediction and ground-truth of HM scan-paths. Then, the \textit{reward} is used to make decision on the \textit{action} through the DRL model, i.e., the HM scan-path at the current frame. In the DRL model, the \textit{observation} of the extracted FoV and the \textit{reward} of HM scan-path prediction compose an \textit{environment}.
...
(1) At frame $t$, the FoV extractor obtains the current $\textit{observation}$ $\mathbf{o}^n_t$ from the input video frame $\mathbf{F}_t$, according to the predicted HM position $(\hat{x}^n_t,\hat{y}^n_t)$.
In our work, $\mathbf{o}^n_{t}$ is the content of FoV ($103^{\circ} \times 60^{\circ}$) in the $n$-th workflow. It is projected to 2D region and then down-sampled to $42\times42$.\\
...
(5) \textit{Environment} is updated with $\hat{\nu}^n_t$ and $\hat{\alpha}^n_t$, leading to $(\hat{x}^n_t, \hat{y}^n_t)\longrightarrow (\hat{x}^n_{t+1},\hat{y}^n_{t+1})$. The FoV extractor returns a new $\mathbf{o}^n_{t+1}$ according to the HM position $(\hat{x}^n_{t+1},\hat{y}^n_{t+1})$, and the \textit{reward} estimator returns the \textit{rewards} $r^{\nu}_{n,t}$ and $r^{\alpha}_{n,t}$ in predicting $\hat{\nu}^n_t$ and $\hat{\alpha}^n_t$, upon the ground-truth HM scan-paths of $\{\nu^m_t\}_{m=1}^{M}$ and $\{\alpha^m_t\}_{m=1}^{M}$.\\
...
Next, we focus on modeling the \textit{reward}: $r^{\alpha}_{n,t}$ and $r^{\nu}_{n,t}$.
When training the DRL model, our goal is to make $\hat{\alpha}^n_{t}$ and $\hat{\nu}^n_{t}$ approach to the ground-truth HM data.
Thus, the  \textit{reward} $r^{\alpha}_{n,t}$ and $r^{\nu}_{n,t}$ can be represented by the differences from $\hat{\alpha}^n_{t}$ and $\hat{\nu}^n_{t}$ to $\{{\alpha}^m_{t}\}_{m=1}^M$ and $\{{\nu}^m_{t}\}_{m=1}^M$, respectively.
In our approach, we use the Gaussian distribution to measure the differences between $\hat{\alpha}^n_{t}$ and the ground-truth $\{\alpha^{m}_{t}\}_{m=1}^M$. In addition, we also consider the distances from predicted HM position $(\hat{x}^n_t,\hat{y}^n_t)$ to $\{(x^{n}_{t},y^{n}_{t})\}_{m=1}^M$ in the \textit{rewards} of $r^{\alpha}_{n,t}$ and $r^{\nu}_{n,t}$, and the distances are modeled by the 2D Gaussian distribution.
...
\end{equation}
In \eqref{reward-alpha}, $D_d$ defines \textit{phase difference}, and $D_s$ denotes \textit{great-circle distance} \cite{shumaker1984astronomical}. Moreover, $\rho$ and $\varrho$ are the standard deviations of Gaussian distributions, as the hyper-parameters. Similarly, we have
\begin{equation}
\label{reward-nu}
...
\textbf{Optimization.}
Now, we need to optimize \textit{reward}: $r^{\alpha}_t$ and $r^{\nu}_t$, when learning the network parameters of our DRL model in Figure \ref{train-framework}.
Our offline-DHP approach implements the asynchronous DRL method \cite{mnih2016asynchronous}.
Hence, multiple workflows are run to interact with multiple \textit{environments} with workflow-specific parameter vectors $\{ \theta^{'}_{\nu}, \theta^{'}_{\pi}, \theta^{'}_{V} \}$, producing $\hat{\nu}^n_t$, $\hat{\pi}^n_t$ and $V$.
...
In our approach, \textit{reward} $r^{\nu}_t$ is optimized to train $\theta_{\nu}$ as follows,
\begin{equation}
\label{opt-1}
d \theta_{\nu} \leftarrow d \theta_{\nu} + \nabla_{\theta_{\nu}^{'}} \sum_{t=1}^{T} r^{\nu}_t.
\end{equation}
...
\begin{equation}
\label{opt-2}
\small d \theta_{V} \leftarrow d \theta_{V} + \nabla_{\theta_{V}^{'}} \sum_{t = 1}^{T} (\sum_{i=t}^{T} \gamma^{i-t} r^{\alpha}_{i} - V(\mathbf{o}^n_{t}, \mathbf{f}^n_{t-1} ; \theta_{V}^{'}))^2,
\end{equation}
\begin{small}
\begin{eqnarray}
\label{opt-3}
\hspace{0.1cm} \nonumber d \theta_{\pi} \leftarrow d \theta_{\pi} +\nabla_{\theta_{\pi}^{'}} \sum_{t = 1}^{T} \log \pi( \hat{\alpha}^n_{t} | \mathbf{o}^n_{t}, \mathbf{f}^n_{t-1} ; \theta_{\pi}^{'})\cdot \\
(\sum_{i = t}^{T} \gamma^{i-t} r^{\alpha}_{i} - V(\mathbf{o}^n_{t}, \mathbf{f}^n_{t-1} ; \theta_{V}^{'})),
\end{eqnarray}
\end{small}
where $\gamma$ is the discount factor of \textit{Q-learning} \cite{watkins1992q}.
In addition, $V(\mathbf{o}^n_{t}, \mathbf{f}^n_{t-1} ; \theta_{V}^{'})$ denotes state value $V$ yielded by $\mathbf{o}^n_{t}, \mathbf{f}^n_{t-1}$ and $\theta_{V}^{'}$; $\pi( \hat{\alpha}^n_{t} | \mathbf{o}^n_{t}, \mathbf{f}^n_{t-1} ; \theta_{\pi}^{'})$ stands for the probability of \textit{action} $\hat{\alpha}^n_{t}$ that is yielded by policy $\pi_t$ from $\mathbf{o}^n_{t}, \mathbf{f}^n_{t-1}$ and $\theta_{\pi}^{'}$.
Finally, upon the above equations, RMSProp \cite{tieleman2012lecture} is applied to optimize \textit{reward} in training data. As a result, the workflow-specific and global-shared parameter vectors can be learned to predict HM scan-paths.
...
Our online-DHP approach refers to predicting the $n$-th subject's HM position $(\hat{x}^n_{t+1},\hat{y}^n_{t+1})$ at the next frame, given his/her HM positions $\{(x^n_{1},y^n_{1}),\ldots, (x^n_{t},y^n_{t})\}$ till the current frame.
...
As shown in this framework, the input to our online-DHP approach is the viewer's HM scan-path $\{(\alpha^n_1,\nu^n_1),\ldots, (\alpha^n_{t-1},\nu^n_{t-1})\}$ and frame content $\{\mathbf{F}_1, \ldots, \mathbf{F}_t \}$ till frame $t$, while the output is the predicted HM position $(\hat{x}^n_{t+1},\hat{y}^n_{t+1})$ at frame $t+1$ for the \textit{viewer}.
This can be seen as online prediction of HM positions $\{(\hat{x}^n_{t},\hat{y}^n_{t})\}_{t=1}^{T}$ .
...
In the following, we discuss these two stages in more details.

\subsection{Stage I: Training}
At the beginning, the HM position $(\hat{x}^n_1,\hat{y}^n_1)$ of the \textit{viewer} is initialized to be the center of front region, which is the general setting of  the panoramic video player. Then, the trained DRL network of our offline-DHP approach is loaded as the initial DRL network for online prediction, and both networks share the same structure.
...
\begin{enumerate}
  \item Iterate the following steps from $i=1$ to $t$. At each iteration, $(\hat{\alpha}_i,\hat{\nu}_i)$ and $(\alpha^n_i,\nu^n_i)$ are the predicted and ground-truth \textit{action} of the HM scan-path, and $\mathbf{o}^n_i$ is the \textit{observation} of the FoV content for the subject.
  \item Take \textit{action} of $(\hat{\alpha}_i,\hat{\nu}_i)$ upon the current \textit{observation} $\{\mathbf{o}^n_1, \ldots, \mathbf{o}^n_i\}$ till frame $i$. The \textit{action} of $\hat{\alpha}_i$ selects 1 among 8 discrete HM scan-path directions, i.e., $\{ 0^{\circ}, 45^{\circ}, \cdots, 315^{\circ} \}$. The \textit{action} of $\hat{\nu}_i$ also contains a scalar of HM scan-path magnitude.
  \item Calculate \textit{reward} $(r^{\alpha}_{n,i}, r^{\nu}_{n,i})$ from the \textit{reward} estimator with \eqref{reward-alpha} and \eqref{reward-nu}, which measures how close the \textit{action} $(\hat{\alpha}^n_i,\hat{\nu}^n_i)$ to the ground-truth HM scan-path $(\alpha^n_i,\nu^n_i)$  of the $n$-th \textit{viewer}. Here, the ground-truth HM scan-path used in \eqref{reward-alpha} and \eqref{reward-nu} is from the single \textit{viewer}, rather than from all subjects.
  \item Generate new \textit{observation} ${\mathbf{o}^n_{i+1}}$ from the FoV extractor with above \textit{action} $(\hat{\alpha}^n_i,\hat{\nu}^n_i)$, and then it is input to the DRL network.
  \item Update the DRL network using \eqref{opt-2} and \eqref{opt-3}, if the iteration number $i$ is equivalent to $t$.
\end{enumerate}
Here,  the definitions of \textit{action}, \textit{reward} and \textit{observation} are the same as Section \ref{train}.
The above iterations share the same implementation of reinforce learning as that of one DRL workflow of offline-DHP, which is presented in Section \ref{train}.
...
In our approach, there are two criteria for the termination condition. The first criterion is the maximal number $E$ of episodes, which is set to be 500 in this paper.
The second one is based on mean overlap (MO), which measures how close the predicted HM position to the ground-truth HM position.
MO ranges from 0 to 1, where larger MO indicates more precise prediction.
...
Algorithm \ref{online-DHP-algorithm} presents the summary of the training stage in online-DHP.

\subsection{Stage II: Prediction}
When the average MO is larger than a threshold $th_{\text{mo}}$, the switch in Figure \ref{online-framework} is turned to "predict" and the DRL network make a prediction for action at frame $t+1$.
...
When entering the prediction stage, the trained model is used to produce the HM position as follows.


With the trained DRL model at the first stage, XXX

The direction and magnitude of HM scan-path, i.e., $\hat{\alpha}_t$ and $\hat{\nu}_t$, can be obtained at frame $t+1$, as the \textit{action} of DRL. As a result, the HM position $(\hat{x}^n_{t+1},\hat{y}^n_{t+1})$ at frame $t+1$ can be predicted, given the ground-truth HM position $(x^n_{t},y^n_{t})$ at frame $t$.. Algorithm XXX presents the summary of the prediction stage in online-DHP. Finally, online-DHP is achieved by training and predicting till the last frame.


\begin{algorithm}
   \caption{The algorithm of online-DHP for subject $m$ when predicting $t+1$: Stage Training.}
   \label{online-DHP-algorithm-training}
\begin{algorithmic}
   \STATE {\bfseries Input:} Panoramic video and viewer's ground-truth HM position $\{(x^{m}_i,y^{m}_i)\}_{i=1}^{t}$;
   ...
   \FOR{$e=1$ {\bfseries to} $E$}
       \STATE Initialize HM position: $\hat{x}_1=0,\hat{y}_1=0$;
       \STATE Initialize LSTM feature: $\mathbf{f}_0=\mathbf{0}$;
       \FOR{$i=1$ {\bfseries to} $t-1$}
           \STATE Extract FoV $o_i$ according to $(\hat{x}_{i},\hat{y}_{i})$;
           \STATE Obtain policy and LSTM feature: $\hat{\pi}_{i}, \mathbf{f}_{i}=\hat{\pi}( \mathbf{o}_{i}, \mathbf{f}_{i-1} ; \theta_{\hat{\pi}})$;
           \STATE Select $\hat{\alpha}_{i}$ according to the $\epsilon$-greedy policy based on $\hat{\pi}_{i}$;
           \STATE Obtain HM magnitude: $\hat{\nu}_{i}=\hat{\nu}(\mathbf{o}_{i}, \mathbf{f}_{i-1} ; \theta_{\hat{\nu}})$;
           \STATE According to $\hat{\alpha}_{i},\hat{\nu}_{i}$, update $(\hat{x}_{i}, \hat{y}_{i})$ to $(\hat{x}_{i+1}, \hat{y}_{i+1})$;
           \STATE Estimate reward $r^{\nu}_{i},\! r^{\alpha}_{i}$ of $\hat{\alpha}_{i},\hat{\nu}_{i}$ according to \eqref{reward-alpha}, \eqref{reward-nu};
           \STATE Compute $\text{MO}_i$ of $(\hat{x}_{i},\hat{y}_{i})$ and $(x^{m}_{i},y^{m}_{i})$;
       ...
       \ENDIF
  \ENDFOR
  \STATE {\bfseries Return:} Trained parameter vectors: $\{ \theta_{\hat{\nu}}, \theta_{\hat{\pi}}, \theta_{V} \}$;
\end{algorithmic}
\end{algorithm}

\begin{algorithm}
   \caption{The algorithm of online-DHP for subject $m$ when predicting $t+1$: Stage Predicting.}
   \label{online-DHP-algorithm-predicting}
\begin{algorithmic}
   \STATE {\bfseries Input:} Trained parameter vectors: $\{ \theta_{\hat{\nu}}, \theta_{\hat{\pi}}, \theta_{V} \}$ from stage training;
   \STATE Load trained parameter vectors from stage training: $\{ \theta_{\hat{\nu}}, \theta_{\hat{\pi}}, \theta_{V} \}$;
   \STATE Initialize LSTM feature: $\mathbf{f}_0=\mathbb{0}$;
   \FOR{$i=1$ {\bfseries to} $t-1$}
       \STATE Extract FoV $\mathbf{o}_i$ according to $(x^{m}_{i},y^{m}_{i})$;
       \STATE Obtain LSTM feature: $\mathbf{f}_{i}=\hat{\pi}( \mathbf{o}_{i}, \mathbf{f}_{i-1} ; \theta_{\hat{\pi}})$;
       \STATE $i \leftarrow i+1$;
   \ENDFOR
   \STATE Extract FoV $\mathbf{o}_t$ according to $(x^{m}_{t},y^{m}_{t})$;
   \STATE Obtain policy: $\hat{\pi}_{t}=\hat{\pi}( \mathbf{o}_{t}, \mathbf{f}_{t-1} ; \theta_{\hat{\pi}})$;
   \STATE Choose $\hat{\alpha}_{t}$ with the greedy policy based on $\hat{\pi}_{i}$;
   \STATE Obtain HM magnitude: $\hat{\nu}_{t}=\hat{\nu}(\mathbf{o}_{t}, \mathbf{f}_{t-1} ; \theta_{\hat{\nu}})$;
   \STATE According to $\hat{\alpha}_{t},\hat{\nu}_{t}$, update $(x^{m}_{t}, y^{m}_{t})$ to $(\hat{x}_{t+1}, \hat{y}_{t+1})$, which is the prediction of HM position at frame $t+1$;
   \STATE {\bfseries Return:} The online prediction of the HM position for subject $m$ at frame $t+1$: $(\hat{x}_{t+1},\hat{y}_{t+1})$;
\end{algorithmic}
\end{algorithm}
